...
from models.E_intrips import E_InTripModel
...
blp = Blueprint("E_Incoming", "E_Incoming", description="Operations on line E Incoming trips")
...
@blp.route("/<int:station_id>/E_intrips/next_1_hour")
class NextHourTrips(MethodView):
    def get(self, station_id):
        ...
        # Determine trip types based on today’s date
        weekday = now.weekday()  # Monday is 0, Sunday is 6
        today_formatted = now.strftime('%d-%m')

        # Fetch Tunisia's holidays
        holidays = get_tunisia_holidays()

        # Determine trip types
        if today_formatted in holidays:
            trip_types = ['C', 'B']
        elif weekday == 6:  # Sunday
            trip_types = ['C', 'B']
        else:
        ...
        trips = E_InTripModel.query.filter(
            E_InTripModel.TripType.in_(trip_types),
            func.strftime('%H:%M:%S', getattr(E_InTripModel, station_name)) >= current_time,
            func.strftime('%H:%M:%S', getattr(E_InTripModel, station_name)) <= one_hour_later
        ).all()
...
@blp.route("/<int:station_id>/E_intrips/rest_of_day")
class TodayTrips(MethodView):
    def get(self, station_id):
        ...
        # Get today's date and trip types
        today = datetime.now()
        trip_types = get_trip_types_for_specific_day(today)  
...
@blp.route("/<int:station_id>/E_intrips/next_day")
class NextDayTrips(MethodView):
    def get(self, station_id):
        ...
        next_day_name = next_day.strftime('%A')  # Full weekday name

        # Use the corrected function to get trip types for the next day
        trip_types = get_trip_types_for_specific_day(next_day)
